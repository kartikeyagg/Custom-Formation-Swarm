...
def check_arm(dronelist):
    for i in dronelist:
        if ( not (i.vehicle.is_armable)):
            return 0
    return 1
def form_vel(dronelist,drn,sysid,neighbors):
...
def follow_trajectories(sysid,drn,dronelist,start_time):
...
if __name__ == '__main__':
    ...
    start_time = time.time()
    for i in range(len(dronelist)):
        thread_list_trajectory.append(threading.Thread(target=follow_trajectories,args=(i+1,dronelist[i],dronelist,start_time)))
